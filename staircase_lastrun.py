...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/niteshshah/Desktop/psycology lab/staircase_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fization = visual.ShapeStim(
        win=win, name='fization', vertices='cross',
        size=(0.1, 0.1),
    ...
    for thisStair in stair:
        ...
        trial = data.Routine(
            name='trial',
            components=[fization, grating, key_resp],
        )
        ...
        while continueRoutine:
            ...
            # if fization is starting this frame...
            if fization.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fization.frameNStart = frameN  # exact frame index
                fization.tStart = t  # local t and not account for scr refresh
                fization.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fization, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fization.started')
                # update status
                fization.status = STARTED
                fization.setAutoDraw(True)
            
            # if fization is active this frame...
            if fization.status == STARTED:
                # update params
                pass
            
            # if fization is stopping this frame...
            if fization.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fization.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
                    fization.tStop = t  # not accounting for scr refresh
                    fization.tStopRefresh = tThisFlipGlobal  # on global time
                    fization.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fization.stopped')
                    # update status
                    fization.status = FINISHED
                    fization.setAutoDraw(False)
